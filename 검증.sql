...
select p.pplname,count(p.pplname), m.korName
from tblExpertScore ps 
join tblppl p on ps.seqppl = p.seqppl group by p.pplname 
join tblMovie m on m.seqM = ps.seqM
order by count(p.pplname) desc;

-- 10. 네티즌 리뷰를 가장 많이 작성한 네티즌의 아이디(이름)를 가져오시오.
select pplname 
from (select p.pplname,count(p.pplname) from tblExpertScore ps join tblppl p on ps.seqppl = p.seqppl group by p.pplname order by count(p.pplname) desc )
where rownum = 1;
